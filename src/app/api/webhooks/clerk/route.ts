// Your local server (on localhost:3000) is not publicy accessible, meaning Clerk can't directly send webhook requests to it. Localhost is only accessible from your own computer, not from the internet.
...
export const POST = async (req: Request) => {
  ...
  switch (evt.type) {
    ...
    case "user.created":
      ...
      if (!id || !email_addresses || !image_url) {
      ...
      if (!existingUser) {
        ...
        try {
          ...
          return new Response(`Creation of user failed! ${err}`, {
            status: 400,
          });
        }
      }
    ...
    case "user.updated": {
      ...
      try {
        ...
        return new Response(`Updating user failed! ${err}`, {
          status: 400,
        });
      }
    ...
    case "user.deleted": {
      ...
      try {
        ...
        return new Response(`Deleting user failed! ${err}`, {
          status: 400,
        });
      }
    }
